...
import { getClient, updateClient, archiveClient } from '@/clients/client-service';
...
export default async function ClientDetailPage({
  ...
  let client: any = null;
  try {
    client = await getClient(db, userId, organizationId, clientId);
  } catch {
    client = null;
  }
  ...
  if (!client) {
    return (
      <main style={{ padding: '2rem', maxWidth: '800px', margin: '0 auto', fontFamily: 'sans-serif' }}>
        ...
        <p>The requested client does not exist or belongs to another organization.</p>
  ...
  async function handleArchiveClient() {
    ...
    await archiveClient(db, uId, orgId, clientId);
    redirect('/clients');
  }
  ...
  async function handleCreateProject(formData: FormData) {
    ...
    const ownerId = String(formData.get('ownerId') || uId);

    const project = await createProject(db, uId, orgId, {
      clientId,
      ownerId,
      name,
      projectType,
